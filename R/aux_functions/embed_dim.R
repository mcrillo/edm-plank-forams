...
embed_dim <- function(data_ts, emb_dim, trap_name, overwrite){    
 ...
 if(overwrite == TRUE | !file.exists(paste("output/",trap_name,"/embedding_plots",sep=""))){
  ...
  for(i in 2:ncol(data_ts)){ # simplex for each variable
    ...
    # maximization by eye, because of sst, G. crassaformis, G. falconensis problematic cases
    Emb1[i,"emax_eye"]<-emax_eye[i-1]
    Emb1[i,"rho_eye"]<-simplex_output[emax_eye[i-1],"rho"]

    # trying to automatise "eye" maximization
    # for rhos within 0.02 difference from the max rho, take min Embedding dimension
    ...
    png(savename, width = 800, height = 600)
      ...
      points(x=Emb1[i,"emax_opt"], y= Emb1[i,"rho_opt"],  type = "b", col  =  "green", lwd=4, pch=19)
      points(x=Emb1[i,"emax_eye"], y= Emb1[i,"rho_eye"],  type = "b", col  =  "blue", lwd=4, pch=19)
      points(max(simplex_output$rho) ~ simplex_output$E[which.max(simplex_output$rho)],  type = "b", col  =  "red", lwd=4, pch=19)
    dev.off()
