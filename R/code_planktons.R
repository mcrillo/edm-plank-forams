...
data_plankton_original_df<-as.data.frame(data_plankton_original) #same as above#
...
plot(data_plankton_original_ts[,c(1:10)],  main = "plot of plankton series") #same as above#
plot(data_plankton_original_ts[,c(11:15)], main = "plot of plankton series") #same as above#
...
cor_plankon_original_ts<-cor(data_plankton_original_ts[,1:15], method = "pearson")
corrplot(cor_plankon_original_ts, method = "square", title = "data_plankton_original_ts")
...
tp_simplex2<-"default"
Emb2<-c() #embedding vector#
sp<-colnames(data_plankton_original_ts) #picking the colnames#
par(mfrow = c(1,1),mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0)) #parameters to the plots#
for(i in 1:15){ ##to run over all species of planktons##
  nome<-sp[i] #name variable to set plot with it#
  lib<-c(1, NROW(data_plankton_original_ts[,i])) #defining the lib size used to simplex algorithmic#
  pred<-lib #same as above#
  simplex_output2<-simplex(as.data.frame(data_plankton_original_ts)[,i], 
                          lib = lib, 
                          pred = pred, 
                          E = 2:10, 
                          silent = TRUE,
                          tp =  tp_simplex2) #simplex method function calling#
  #par(mfrow = c(5,2),mar = c(4, 4, 1, 1), mgp = c(2.5, 1, 0))
  #savename<-paste("~/Dropbox/plankton_marina/data/embeddings/embedding_original_",sep = "", nome, tp_simplex2, ".png") #saving name to be set on the archive and as title of the plot#
  #png(savename, width = 800, height = 600) #png ambient to save the figure as a png figure#
  plot(simplex_output2$E, simplex_output2$rho, type = "b", xlab = "Dimensão de Embedding (E)", 
       ylab = expression(paste(sep = "",rho)), main = paste(nome, sep = "_tp_",tp_simplex2), cex.lab=1.5) #plot function with extra parameters to plotting#
  emax<-which.max(simplex_output2$rho) #picking the maximum of embedding dimension#
  lines(rho[emax] ~ E[emax], data = simplex_output2, type = "b", col  =  "red", lwd=2, pch=19) #setting the maximum emebedding dimension on the plot#
  Emb2[i]<-emax #placing the maximum into the embedding vector for the record and future uses#
  #dev.off() #shutting down the png ambient#
}
...
for (i in 2:16) {
  ...
  #savename<-paste("~/Dropbox/plankton_marina/data/Theta/theta_",sep = "", nome, ".png") #saving name to be set on the archive and as title of the plot#
  #png(savename, width = 800, height = 600) #png ambient to save the figure as a png figure#
...
for (i in 1:15) {
  ...
  pred<-lib
  s_map_output<-s_map(as.data.frame(data_plankton_original_ts)[,i], 
                      lib = lib, 
                      pred = pred, 
                      E = Emb2[i], 
                      theta = c(0, 1e-04, 3e-04, 0.001,0.003, 0.01, 0.03, 0.1, 0.3, 0.5, 0.75, 1, 1.5, 2, 3, 4, 6, 8), 
                      silent = TRUE, 
                      save_smap_coefficients = TRUE)
  #savename<-paste("~/Dropbox/plankton_marina/data/Theta/theta_original_",sep = "", nome, ".png") #saving name to be set on the archive and as title of the plot#
  #png(savename, width = 800, height = 600) #png ambient to save the figure as a png figure#
  plot(s_map_output$theta, s_map_output$rho, type = "b", xlab = expression(paste("Theta (", sep = "", theta, ")")), 
       ylab = expression(paste(sep = "",rho)), main = nome , cex.lab = 1.5)
  thetamax<-which.max(s_map_output$rho)
...
##DATA_PLANKTON_ORIGINAL_TS###
E_n2<-as.data.frame(matrix(data = NA, nrow = 14, ncol = 15))
sp<-colnames(data_plankton_original_ts)
colnames(E_n2)<-sp[1:15]
for (i in 2:15) { ##To run over each species##
  for (j in 1:15) { ##To run over each species differing from the first one##
    if(i == j)next
    sp<-colnames(data_plankton_original_ts)
    E_max_ccm2<-data.frame()
    #E_max_ccm<-colnames(sp[2:16])
    E.test.2=NULL
    par(mfrow = c(1,1), mar = c(4,4,1,1), mgp =  c(2.5,1,0))
    cmxy.t2<-list()
    for(E.t2 in 2:10){ ##to run over different embedding dimensions##
      cmxy.t2 <- ccm(data_plankton_original_ts, E = E.t2, lib_column = sp[i], target_column = sp[j],
                    lib_sizes = 238, num_samples = 1, tp=-1,random_libs = F, silent = TRUE)
      E.test.2=rbind(E.test.2,cmxy.t2)
    }
    E_n2[i, j] <- which.max(E.test.2$rho) # the optimal E
    plot(E.test.2$E, E.test.2$rho, type = "b", 
         ylab = expression(paste(sep = "", rho)), 
         xlab = "dimensão de Embedding (E)",
         main = paste(sp[i], sep = "_", "xmap", sp[j], "original"))
    lines(rho[E_n2[i, j]]~E[E_n2[i, j]], data = E.test.2, type = "b", col =  "red", lwd = 2, pch = 19)
    
  }
}

# CCM analysis with varying library size (L)
#libs<-c(1, NROW(data_plankton_original_ts[,i]))
##CCM from each species to SST###

par(mfrow = c(1,1), mar = c(4,4,1,1), mgp =  c(2.5,1,0))
for (i in 2:15) { ##To run over each species##
  spi_xmap_spj <- ccm(data_plankton_original_ts, E=E_n2[i, j],lib_column=sp[i], target_column="sst",
                      lib_sizes=seq(1,238, by = 10), replace=T, silent = TRUE)
  ...
  plot(spi_xmap_spj_means$lib_size, pmax(0, spi_xmap_spj_means$rho), 
       ...
       ylab = expression(paste("Mapping skil (", sep = "", rho, ")")),
       col = "red", 
       main = paste(sp[i], sep = "_", "xmap_sst_original"),
       lwd = 2, 
       pch = 19, 
       cex.lab = 1.5)
  #abline()
...
for (i in 2:15) { ##To run over each species##
  for (j in 2:15) {
    if(i == j)next
    spi_xmap_spj <- ccm(data_plankton_original_ts, E=E_n2[i, j],lib_column=sp[i], target_column=sp[j],
                        lib_sizes=seq(1,238, by = 10), replace=T, silent = TRUE)
    ...
    plot(spi_xmap_spj_means$lib_size, pmax(0, spi_xmap_spj_means$rho), 
         ...
         ylab = expression(paste("Mapping skil (", sep = "", rho, ")")),
         col = "red", 
         main = paste(sp[i], sep = "_", "xmap", sp[j], "_original"),
         lwd = 2, 
         pch = 19, 
         cex.lab = 1.5)
    #abline()
